<template>
     <div class="pbox">
        ...
        <li v-for="item in data" class="li">
            <router-link :to="'/list/'+item.pid" v-if="item.son">{{item.pname}}</router-link>
...
<script>
    export default {
        ...
        created(){
            this.$store.commit("setAble",true);
        ...
        methods:{
            ...
            Part(obj){
                ...
                obj.style = {color:'red'}
                var pid=obj.pid;
                this.$store.commit("setAble",false);
                this.$store.commit('setUPid',pid);
            }
        }
    }
</script>
